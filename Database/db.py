...
class Database:
    def __init__(self, uri, database_name):
        ...
        self.blocklists = self.db.blocklists
    ...
    # --- APPROVALS & BLOCKLIST ---
    async def approve_user(self, chat_id, user_id): await self.approvals.update_one({"_id": f"{chat_id}_{user_id}"}, {"$set": {"chat_id": chat_id, "user_id": user_id}}, upsert=True)
    async def unapprove_user(self, chat_id, user_id): await self.approvals.delete_one({"_id": f"{chat_id}_{user_id}"})
    ...
    async def add_warn(self, chat_id, user_id):
        res = await self.warns.find_one_and_update({"_id": f"{chat_id}_{user_id}"}, {"$inc": {"count": 1}}, upsert=True, return_document=True)
        return res.get("count")
    async def reset_warns(self, chat_id, user_id): await self.warns.delete_one({"_id": f"{chat_id}_{user_id}"})

    # --- FEDERATIONS ---
    async def create_fed(self, fed_id, name, owner_id): await self.federations.insert_one({"_id": fed_id, "name": name, "owner": owner_id, "banned": []})
    async def join_fed(self, chat_id, fed_id): await self.groups.update_one({"_id": chat_id}, {"$set": {"fed_id": fed_id}})
    async def get_chat_fed(self, chat_id): res = await self.groups.find_one({"_id": chat_id}); return res.get("fed_id") if res else None
    async def fban_user(self, fed_id, user_id): await self.federations.update_one({"_id": fed_id}, {"$addToSet": {"banned": user_id}})
    async def is_fbanned(self, fed_id, user_id):
        res = await self.federations.find_one({"_id": fed_id, "banned": user_id})
        return bool(res)
